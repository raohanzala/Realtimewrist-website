...
import NavListBlack from './NavListBlack'



const NavbarBlack = ({ showSearch, setShowSearch }) => {
  ...
  return (
    <div className={`flex flex-col bg-[#232323] w-full fixed shadow-md z-[99] transition-transform duration-300 ease-in-out ${scrolled ? '-translate-y-full' : 'translate-y-0'}`}>
      <TopBar />
      <LogoBlack />

      {/*  ------------- Social Icons ----------------- */}
      <div className='flex items-center justify-between p-2 sm:p-3 px-5 font-medium bg-white'>
        <div onClick={() => setVisible(true)} className='cursor-pointer pl-2 text-[#333] flex md:hidden text-2xl'>
          <MdOutlineMenu />
        </div>
        <div className='text-xl  text-[#333] hidden md:flex gap-3 cursor-pointer '>
          <CiFacebook />
          <IoLogoInstagram />
          <AiOutlineYoutube />
          <FaWhatsapp />
        ...
        <div className='flex items-center'>
          <div onClick={() => setShowSearch(true)} className='cursor-pointer text-[#333] text-xl pr-2'>
            <FiSearch />
          </div>
