...
import { clearCart, closeCart, removeFromCart } from '../store/slices/cartSlice';
...
const CartDrawer = () => {
  ...
  const ordes = useUserCart()


  console.log(ordes, 'DATA CART ITEMS')
      ...
      >
        <div className='flex flex-col justify-between h-full'>
          <div className="flex items-center justify-between p-4 border-b">
            ...
            <h2 className="text-sm" onClick={()=> dispatch(clearCart())}>Clear Cart </h2>
            <button onClick={() => dispatch(closeCart())} className="text-gray-500 hover:text-gray-900">
                    ...
                    <div key={index} className="p-2 rounded-sm my-3 border text-gray-700 flex relative items-center gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-semibold">{CURRENCY}{item?.newPrice}</p>
                          
                        </div>
                      </div>
                      <QuantityInput item={item}/>
                      <button className='absolute -top-1 -right-1 bg-white '>
                        <ImCancelCircle size={14} className='text-gray-500 hover:text-black' onClick={() => dispatch(removeFromCart({itemId: item._id}))} />
                      </button>
                    </div>
                );
              })
          ...
          <div className="sm:p-4 p-2 border-t mt-auto">
            ...
            <div className="flex sm:gap-4 gap-2 mt-4">
              <Button variant='secondary' className={`w-full relative`} onClick={handleViewCart} startIcon={<IoMdCart className='text-xl' />}>
                  View Cart
                <p className='absolute right-[-5px] -top-1 w-4 text-center leading-4 bg-[red] text-white aspect-square rounded-full text-[8px]'>{totalItems}</p>
              </Button>
              <Button
